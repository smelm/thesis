...
There are a few different possibilities to avoid this. One would be to replace \pattern{.} with \pattern{[\caret "]} which matches anything but quotes. When this is not feasible -- which may be the case in more complex patterns -- a lazy quantifier \pattern{".*?"} can be used (Listing \ref{fig:regexSurroundingCorrectLazy}). 
...
Sometimes, the input contains interdependencies. This can be observed when there are different options for the same delimiter like double and single quotes. When the pattern \pattern{("|').*?("|')} is applied to an input with nested quotes, it matches them incorrectly (Listing \ref{fig:regexNestedQuotes}).
